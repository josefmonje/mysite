...
aws cloudwatch put-metric-alarm --alarm-name status-mon-1 --alarm-description "Alarm when Prod 1 isn't running" --metric-name StatusCheckFailed --namespace AWS/EC2 --statistic Maximum --period 60 --threshold 1 --comparison-operator GreaterThanOrEqualToThreshold --dimensions "Name=InstanceId,Value=$instanceId2" --evaluation-periods 1 --alarm-actions $snsArn1 --unit Count
aws cloudwatch put-metric-alarm --alarm-name status-mon-2 --alarm-description "Alarm when Prod 2 isn't running" --metric-name StatusCheckFailed --namespace AWS/EC2 --statistic Maximum --period 60 --threshold 1 --comparison-operator GreaterThanOrEqualToThreshold --dimensions "Name=InstanceId,Value=$instanceId3" --evaluation-periods 1 --alarm-actions $snsArn1 --unit Count
