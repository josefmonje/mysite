...
echo "Creating key"
aws ec2 create-key-pair --key-name $keyPairName --query 'KeyMaterial' --output text > $keyFile
chmod 400 $keyFile

echo "Setting up Dev Server..."
instanceId1=`aws ec2 run-instances --image-id $amiId --count 1 --instance-type t2.nano --key-name $keyPairName --security-group-ids $securityGroupId1 --subnet-id $subnetId --associate-public-ip-address --query 'Instances[0].InstanceId' --output text`
...
cat $myPubKey | ssh -i $keyFile ubuntu@$instance1Url 'cat >> ~/.ssh/authorized_keys'
sleep 5
...
devIp=`ssh -i $keyFile ubuntu@$instance1Url 'curl http://instance-data/latest/meta-data/public-ipv4'`
...
scp -i $keyFile ~/.aws/* ubuntu@$instance1Url:~/.aws/.
